"""K sweep with the user's GUI conditions (phi=0.5, EGR=0.3, CR=12.5)."""
import numpy as np
import time
import warnings
...
print("="*60)
print("Reference: Single-zone (GUI conditions: phi=0.5, EGR=0.3, CR=12.5)")
print("="*60)
...
# K sweep
K_values = [1e-6, 2e-6, 3e-6, 5e-6]

print(f"\n{'='*60}")
print(f"K Sweep: {nzones} zones, GUI conditions")
print(f"{'='*60}")
print(f"{'K':>10s}  {'Status':>10s}  {'Peak P':>10s}  {'dP vs SZ':>10s}  {'Peak T':>10s}  {'Time':>8s}")
print("-" * 70)
...
for K in K_values:
    ...
    try:
        ...
        elapsed = time.time() - t0
        print(f"{K:>10.1e}  {'ERROR':>10s}  {'--':>10s}  {'--':>10s}  {'--':>10s}  {elapsed:>6.1f} s")
